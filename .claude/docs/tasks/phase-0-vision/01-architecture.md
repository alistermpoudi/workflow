...
│   ├── DecisionsLog.js       # Écriture + lecture active decisions.log
│   ├── ContextManager.js     # Hiérarchie de chargement contexte LLM
│   ├── PhaseManager.js       # Orchestre les 5 phases (Discovery → Réalisation)
│   └── SyncChecker.js        # Détection state drift + vérification branche Git
│
├── phases/
│   ├── DiscoveryPhase.js     # Questions → vision.md
│   ├── SpecificationPhase.js # Propositions → features.json
│   ├── ValidationPhase.js    # Tâches → versions/ (avec règle de granularité)
│   ├── ArchitecturePhase.js  # Stack → tech-stack.json + TASK-001/002
│   └── ExecutionPhase.js     # Code → validate → corriger (consulte decisions.log)
│
├── tools/
...
│   └── TaskManager.js        # CRUD TASK-XXX.md + progress.json
│
├── interfaces/
│   ├── CLI.js                # readline + chalk (MVP)
...
└── llm/
    ...
    └── PromptBuilder.js      # Construit les prompts avec contexte projet injecté
```
...
│
├── decisions.log
│   # Journal actif — une entrée par décision technique
...
class ContextManager {
  ...
  async loadTaskContext(taskId) {
    ...
    const recentDecisions = await DecisionsLog.getRecent(5);
    return { task, relevantFiles, recentDecisions };
  }
...
class ExecutionLoop {
  async executeTask(task) {
    // Consulter decisions.log avant de commencer
    const decisions = await DecisionsLog.getRelevant(task);
    const context = await ContextManager.loadTaskContext(task.id);
    ...
    while (attempts < 3) {
      ...
      if (!buildResult.success) {
        DecisionsLog.logAttempt(task.id, attempts, buildResult.error);
        continue; // retenter
      }

      const testResult = await this.runTests();
      if (!testResult.success) {
        DecisionsLog.logAttempt(task.id, attempts, testResult.error);
        continue; // retenter
      }
